...
//!
//! An anchored overlay — [`crate::info`]'s popover here — has to be told
...
//!
//! `popover` takes its anchor as a *parameter* rather than owning a trigger
//! itself — it has no way to guess which node the caller meant — so the
//! application supplies it. This module is that seam: name a trigger with
//! [`silka_core::view::Builder::key`], hand [`track`] that key and a signal,
//! and after every layout the signal holds the trigger's rect.
//!
...
//! widget crate would have to invent and maintain. (`examples/roster` needed
//! the identical seam for its hover card; this is the same module, unchanged
//! in everything but the doc comment above.)
