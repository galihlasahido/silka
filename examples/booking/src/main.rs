//! # silka-booking
//!
//! A room-booking form, built to give three `silka-widgets` components
//! their first real caller outside their own gallery card.
//!
//! ```text
//! cargo run -p silka-booking
//! cargo run -p silka-booking -- --preset tailwind --appearance dark
//! ```
//!
...
//! |---|---|
//! | [`silka_widgets::combo_box()`] | [`form::room_combo`] — a searchable pick from ten rooms, filtered by the application exactly as the widget's own docs say it must be |
//! | [`silka_widgets::date_picker()`] | [`form::date_field`] — the booking's day |
//! | [`silka_widgets::popover()`] | [`crate::app`]'s room-details popover — the selected room's floor, capacity and amenities, anchored to a button beside the room field |
//!
