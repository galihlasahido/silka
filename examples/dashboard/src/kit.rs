...
use silka_widgets::{
    button_variant, card_padded, center, icon, spacer, text, BadgeTone, ButtonVariant, CardStyle,
    CardSurface, CardVariant, IconName,
};
...
pub fn card(t: &Theme, label: Option<&str>, children: impl IntoIterator<Item = View>) -> View {
    ...
    let mut c = silka_widgets::card(children).padding_raw(Insets {
        top: 0.0,
        right: 0.0,
...
pub fn padded_card(
    ...
    let mut c = card_padded(children);
    if let Some(label) = label {
        c = c.label(label);
    }
    c.into()
...
///
/// The hairline used to be a second `divider()` written at every call site,
/// which is exactly how a header and the rows under it end up two points
/// apart: [`silka_widgets::card_header`] owns it now.
pub fn card_header(_t: &Theme, title: &str, action: &str, on_action: impl Fn() + 'static) -> View {
    silka_widgets::card_header(title)
        .trailing(button_variant(action, ButtonVariant::Link).on_press(on_action))
        .into()
}
...
///   *status*.
pub fn badge(_t: &Theme, status: Status) -> View {
    let tone = match status {
        Status::Pending => BadgeTone::Warning,
        Status::Success => BadgeTone::Success,
    };
    silka_widgets::badge(status.label())
        .tone(tone)
        .soft()
        .dot(true)
...
pub fn avatar(_t: &Theme, name: &str, diameter: f32) -> View {
    silka_widgets::avatar(name).size_raw(diameter).into()
}
...
pub fn icon_button(
    ...
    interactive(constrained(
        ...
        center(icon(symbol).md().color(ColorToken::SecondaryLabel)),
    ))
    .label(label)
...
pub fn list_row(t: &Theme, primary: &str, secondary: &str, trailing: View) -> View {
    row([
        column([
            View::from(
                text(primary)
...
pub fn trailing_text(t: &Theme, value: &str) -> View {
    text(value)
        .size(t.typography.footnote.size)
...
mod tests {
    ...
    fn the_dashboard_only_maps_its_own_domain_onto_a_tone() {
        ...
        // Built **outside** a build pass, so the theme is installed by hand:
        // the short constructors read the ambient one, and a test that skipped
        // this would silently be asserting against the default preset.
        let pending = silka_core::view::with_theme(t, || {
            silka_widgets::badge(Status::Pending.label())
